package com.tmobile.mytmobile.echolocate.configmanager
...
import com.tmobile.mytmobile.echolocate.configuration.utils.ConfigurationSharedPreference
 ...
 */
class ConfigProvider private constructor(val context: Context) : IConfigProvider {
    ...
    override fun initConfigModule(context: Context) {
        if (configManager == null) {
            ...
            configManager?.getConfigFromServer(0, scheduleJob = false, forceFetch = false)
        }

        // For debug, please check if the configuration file is available in cache folder
        configManager?.getConfigFromCacheFolder()
    ...
    override fun initConfigPreferences(context: Context) {
        ConfigurationSharedPreference.init(context, BuildConfig.FLAVOR)
    }
    ...
    override fun setClientAppPreferences(token: String?, clientAppConfigurationUrl: String, clientAppEnvironment: String, clientAppBuildTypeDebug: Boolean) {
        ConfigurationSharedPreference.tokenObject = token
        ConfigurationSharedPreference.clientAppConfigurationUrl = clientAppConfigurationUrl
        ConfigurationSharedPreference.clientAppEnvironment = clientAppEnvironment
        ConfigurationSharedPreference.clientAppBuildTypeDebug = clientAppBuildTypeDebug
    }
    ...
    override fun clearConfigFromCache() {
        ConfigManager.getInstance(context).clearConfigFromCache()
    }
    ...
    override fun getAvailableConfiguration(): String? {
        return ConfigManager.getInstance(context).getAvailableConfiguration()
    }
    ...
    override fun uploadConfigFromCache(newConfig: String) {
        ConfigManager.getInstance(context).uploadConfigFromCache(newConfig)
    }
    ...
    override fun parseResponse(
        ...
        return ConfigManager.getInstance(context).parseResponse(it, scheduleJob, previousInterval)
    }
    ...
    override fun clearConfigPreferences() {
        ConfigurationSharedPreference.configurationObject = null
        ConfigurationSharedPreference.cacheConfigurationObject = null
        ConfigurationSharedPreference.localConfigurationObject = null
    }
}