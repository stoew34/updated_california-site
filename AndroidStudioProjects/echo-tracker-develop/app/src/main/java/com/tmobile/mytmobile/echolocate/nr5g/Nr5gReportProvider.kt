 ...
 */
class Nr5gReportProvider private constructor(val context: Context) : IBaseReportDataRequestor {
    /** A companion object can access the private members of its companion.*/
    companion object {
        @SuppressLint("StaticFieldLeak")
        @Volatile
        private var INSTANCE: Nr5gReportProvider? = null

        /***
         * creates [Nr5gReportProvider] instance
         */
        fun getInstance(context: Context): IBaseReportDataRequestor {
            return INSTANCE ?: synchronized(this) {
                val instance = Nr5gReportProvider(context)
                INSTANCE = instance
                instance
            }
        }
    ...
    override fun getReports(startTime: Long, endTime: Long): List<ClientSideEvent> {
        ...
        if (nr5gModuleProvider.isNr5gModuleReady() || nr5gModuleProvider.isNsa5gSupported)
         {
                    ...
                    it.forEach {
                        if (it.DIAReportResponseParameters.requestReportStatus == ReportStatusFromModules.STATUS_COMPLETED) {
                            ...
                            val nr5gClientSideEvent =
                                Nr5gUtils.createNr5gClientSideEvent(
                                    nr5gReportPayload)
