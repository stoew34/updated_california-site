...
class Nr5gStatusProcessorTest {

    private lateinit var context: Context
    private lateinit var nr5gDao: Nr5gDao
    private lateinit var db: EchoLocateNr5gDatabase
    private lateinit var dataCollectionService: Nsa5gDataCollectionService
    ...
    fun testGetNr5gStatusProcessor() {
        ...
        val nr5gStatusEntity1 = Nr5gStatusEntity(
            99
        )

        val processor = Nsa5gStatusProcessor(context)

        dataCollectionService = mockkClass(Nsa5gDataCollectionService::class)
        processor.dataCollectionService = dataCollectionService
        every {
            dataCollectionService.getNrStatus(context)
        } returns Nr5gStatusEntity(99)
        ...
        val nr5gStatusEntity2 = nr5gDao.getNr5gStatusEntity(sessionId)
        latch.await(1, TimeUnit.SECONDS)
        Assert.assertNotNull(nr5gStatusEntity2)
    }
}