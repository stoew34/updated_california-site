...
import com.tmobile.mytmobile.echolocate.coverage.model.CoverageSettings
...
class CoverageSettingsProcessorTest {
    ...
    @Test
    fun testCoverageSettingsProcessor() {
        ...
        every {
            coverageNetworkDataCollector.readVoiceCallType(context)
        } returns 3
        every {
            coverageNetworkDataCollector.isRoamingDataEnabled(context)
        } returns true

        val processor = CoverageSettingsProcessor(context)
        processor.networkDataCollector = coverageNetworkDataCollector


        val coverageSettingsEntity1 = CoverageSettingsEntity(
            "type",
            "data"
        )
        ...
        val coverageSettingsEntity2 =
            coverageDao.getCoverageSettingsEntityBySessionID(sessionId)
        Assert.assertNotNull(coverageSettingsEntity2)
