...
/**
 * this class is responsible to creates the nr5gUiLog object and asserts the data
 */
class Nr5gUiLogProcessorTest {
    ...
    fun testInsertNr5gUiLogEntity() {
        ...
        val nr5gUiLogEntity1 = Nr5gUiLogEntity(
            getTriggerTimeStamp(),
            10,
            "abc",
            "abc",
        ...
        val baseNr5gMetricsData = BaseNr5gMetricsData(nr5gUiLogEntity1, "", Nr5gBaseDataMetricsWrapper.ApiVersion.UNKNOWN_VERSION, "")
        val baseNr5gData = BaseNr5gData(sessionId, UUID.randomUUID().toString())

        Nsa5gUiLogProcessor(context).processNr5gMetricsData(baseNr5gMetricsData, baseNr5gData)
        latch.await(1, TimeUnit.SECONDS)

        val nr5gUiLogEntity2 = nr5gDao.getNr5gUILogEntity(sessionId)
        latch.await(1, TimeUnit.SECONDS)
        Assert.assertNotNull(nr5gUiLogEntity2)
    }
